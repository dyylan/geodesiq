...
class Optimizer:
    ...
    def __init__(self, 
        ...
        if commute:
            self.free_indices, self.commuting_ansatz_matrix = commuting_ansatz(target_unitary, 
                                                                               full_basis,
        ...
        if init_parameters is None:
            self.init_parameters = prepare_random_parameters(self.free_indices, 
        ...
        compute_matrix_fn = Optimizer.get_compute_matrix_fn(commuting_ansatz_matrix=self.commuting_ansatz_matrix_free,
                                                            basis=self.projected_basis.basis)
        project_omegas_fn = Optimizer.get_project_omegas_fn(self.full_basis.basis, self.full_basis.dim)
        Udagger_dU_contraction = Optimizer.get_Udagger_dU_contraction_fn()
        fidelity = Optimizer.get_fidelity_fn(self.projected_basis.basis, self.target_unitary)
    ...
    def update_step(self, step_count=(None, None)):
        ...
        if fidelity_new_phi > self.precision:
            ...
            fidelity_phi, fidelity_new_phi, new_phi_ham, step_size = self._new_phi_full(phi_ham, 
                                                                                        proj_c, 
                                                                                        step_size=self.gram_schmidt_step_size)
